...
export function CustomerDetailsDialog({
  ...
  React.useEffect(() => {
    if (!open) setAvatarPreviewOpen(false);
  }, [open]);
  ...
  const avatar = (
    <Avatar size="lg" className={hasAvatar ? "cursor-pointer" : undefined}>
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-3">
            {hasAvatar ? (
              <button
                ...
                aria-label={t("customers.viewPhoto")}
              >
                {avatar}
              </button>
            ) : (
      ...
      <Dialog open={avatarPreviewOpen} onOpenChange={setAvatarPreviewOpen}>
        <DialogContent className="gap-0 overflow-hidden border-0 bg-transparent p-0 shadow-none ring-0 sm:max-w-3xl">
          <DialogHeader className="sr-only">
            <DialogTitle>{fullName}</DialogTitle>
            <DialogDescription>{t("customers.viewPhoto")}</DialogDescription>
          </DialogHeader>
